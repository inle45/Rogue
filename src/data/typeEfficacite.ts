// Table de multiplicateurs de types Pokémon (simplifié Gen 1-6)
// Format: attaquant -> defenseur -> multiplicateur

export const TABLE_EFFICACITE: Record<string, Record<string, number>> = {
  fire: {
...
export function getMultiplicateur(typeAttaque: string, typesDefenseur: string[]): number {
  let multi = 1;
  const tableAttaque = TABLE_EFFICACITE[typeAttaque] || {};
  for (const typeD of typesDefenseur) {
    multi *= tableAttaque[typeD] ?? 1;
  }
  return multi;
}
